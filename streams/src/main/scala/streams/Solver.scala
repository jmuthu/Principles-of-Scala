...
trait Solver extends GameDef {
  ...
  def from(initial : Stream[(Block, List[Move])],
    ...
    case _ ⇒ {
      val exp = initial.foldLeft(explored) { case (e, (block, moves)) ⇒ e + block }
      if (exp.exists(done(_)))
        initial
      else {
        val neighborList = for {
          (block, moves) ← initial
          y ← newNeighborsOnly(neighborsWithHistory(block, moves), exp)
        } yield y
        from(neighborList, exp)
      }
    }
  }
  ...
  lazy val solution : List[Move] = pathsToGoal match {
    //    println(pathsToGoal.head._1, " ", pathsToGoal.head._2.reverse)
